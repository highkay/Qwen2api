...
<p align="center">
  Qwen AI 逆向网关 -- 标准 OpenAI / Anthropic API 兼容
</p>
...
- **多协议兼容** -- OpenAI Chat Completions / Responses API / Anthropic Messages
...
- **多模态输入** -- 支持图片、文档、音频、视频文件输入，视频会按 Qwen Web 的临时 OSS 文件流程传入模型
...
| 协议 | 端点 | 功能 |
|------|------|------|
| OpenAI | `POST /v1/chat/completions` | 聊天补全（流式/非流式/工具调用/思考模式/文件输入） |
| OpenAI | `POST /v1/responses` | Responses API（Codex / Agents / 文件输入） |
| OpenAI | `POST /v1/images/generations` | 图片生成 |
| OpenAI | `POST /v1/images/edits` | 图片编辑 |
| OpenAI | `POST /v1/embeddings` | 未实现，当前返回 501 |
| OpenAI | `GET /v1/models` | 模型列表 |
| Anthropic | `POST /v1/messages` | Claude 兼容（tool_use / thinking / 文件输入） |
...
Chat Completions、Responses、Anthropic Messages 都会从消息内容中提取文件，先通过 Qwen Web 的 STS 接口上传到临时 OSS，再把 `files` payload 传给 Qwen。视频 MIME 当前支持 `video/mp4`、`video/webm`、`video/quicktime`、`video/x-matroska`。

OpenAI Chat Completions 的视频输入示例：
      ...
      "content": [
        {"type": "text", "text": "请详细描述这个视频的内容。"},
        {
          ...
          "file": {
            ...
            "mime_type": "video/mp4",
            "data": "<base64 或 data:video/mp4;base64,...>"
          }
        }
      ]
    }
