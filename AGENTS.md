...
- 多模态文件输入：Chat Completions、Responses、Anthropic Messages 可提取图片、文档、音频、视频并上传到 Qwen 临时 OSS 后传入上游。

当前实现边界：

- `backend/api/embeddings.py` 只是 501 stub，未实现 embeddings。
- 当前磁盘代码没有 `backend/api/gemini.py`，`backend/main.py` 也未注册 Gemini 路由。涉及 Gemini 时先补证或补实现。
- `backend/core/browser_engine.py` 当前是 stub，内部委托 `HttpxEngine`，不是实际 Camoufox 浏览器会话。Dockerfile 默认只安装注册链路需要的 Playwright Chromium headless shell，不下载 Camoufox 浏览器。
...
- 上游访问：`curl_cffi` Chrome TLS 指纹直连、`httpx`、Playwright 注册链路依赖。
...
- `backend/services/file_uploader.py`：多模态文件提取、STS 获取、OSS 上传、解析等待、Qwen Web 兼容 `files` payload 构造，支持图片、文档、音频、视频。
...
1. `backend/api/chat.py`、`backend/api/responses.py`、`backend/api/anthropic.py` 从各自协议的消息内容中提取文件块。
2. `backend/services/file_uploader.py` 统一处理文件：解析 base64/data URI/URL，获取 Qwen STS，上传到临时 OSS，触发解析并等待状态。
3. MIME 分类由 `file_uploader.py` 维护：图片映射为 `vision`，文档映射为 `document`，音频映射为 `audio`，视频映射为 `video`。
4. 视频 payload 必须保持 Qwen Web 形态：`type/showType=video`、`file_class=video`、`greenNet=greening`、`itemId`、`uploadTaskId`、`progress=100`、`status=uploaded`。
5. `backend/services/qwen_client.py` 构建上游 payload 时，顶层 `chat_mode` 对普通文本/文件会话使用 Qwen Web 的 `normal`，消息内 `chat_type/sub_chat_type` 保留内部 `t2t`。
6. 2026-05-28 已用线上实例和样例视频 `1704784424892.mp4` 通过 `/v1/chat/completions` 回归，模型可返回该美团买药呼吸道快检采样说明视频的详细描述。
...
当前仓库已有少量 unittest 风格的 `tests/`，覆盖仍不完整。新增业务逻辑时，如果已有测试不足，应把调查阶段的脚本沉淀成可重复测试或至少保留清晰的验证命令与输入输出证据。
...
- 改多模态文件或视频链路时，同时检查 Chat、Responses、Anthropic、`file_uploader.py`、`qwen_client.py` 的 `chat_mode`/`files` payload，并用真实视频文件回归。
